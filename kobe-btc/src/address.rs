...
impl BtcAddress {
        ...
        match format {
            ...
            AddressFormat::P2WPKH => {
                if !public_key.is_compressed() {
                    return Err(Error::StaticMessage("P2WPKH requires compressed public key"));
                }
                let hash = public_key.hash160();
                let mut data = [0u8; 32];
                data[..20].copy_from_slice(&hash);
            ...
            AddressFormat::P2SH_P2WPKH => {
                if !public_key.is_compressed() {
                    return Err(Error::StaticMessage("P2SH-P2WPKH requires compressed public key"));
                }
            ...
            _ => Err(Error::StaticMessage("Unsupported address format for public key")),
        }
    }
    ...
    fn to_base58check(&self) -> String {
        let prefix = match self.format {
            ...
            AddressFormat::P2SH | AddressFormat::P2SH_P2WPKH => self.network.p2sh_prefix(),
    ...
    fn to_bech32(&self) -> Result<String> {
        ...
        let hrp = Hrp::parse(self.network.bech32_hrp())
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        #[cfg(feature = "alloc")]
        {
            let s = match self.format {
                AddressFormat::P2PKH | AddressFormat::P2SH | AddressFormat::P2SH_P2WPKH => {
                    self.to_base58check()
                }
...
impl core::str::FromStr for BtcAddress {
    ...
    fn from_str(s: &str) -> Result<Self> {
        ...
        if s.starts_with("bc1") || s.starts_with("tb1") || 
           s.starts_with("BC1") || s.starts_with("TB1") {
...
impl BtcAddress {
    ...
    fn from_bech32(s: &str) -> Result<Self> {
        ...
        let (hrp, witness_version, witness_program) = if let Ok(checked) = CheckedHrpstring::new::<Bech32>(s) {
            let hrp = checked.hrp();
...
mod tests {
    ...
    fn test_p2wpkh_address() {
        ...
        let key = BtcPrivateKey::from_bytes(&bytes).unwrap();
        let addr = key.address(Network::Mainnet, AddressFormat::P2WPKH).unwrap();
    ...
    fn test_p2sh_p2wpkh_address() {
        ...
        let key = BtcPrivateKey::from_bytes(&bytes).unwrap();
        let addr = key.address(Network::Mainnet, AddressFormat::P2SH_P2WPKH).unwrap();
        let addr_str = addr.to_string();
